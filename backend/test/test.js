...
station.addStation = async function(req, res) {
  ...
  let station = await stationController.addStation(
...
station.deleteStation = async function(req, res) {
  let station = await stationController.deleteStation(
    req.body.station_id,
    req.body.user_id,
  );
  res.status(200).json(station);
...
station.setIsPrivateOfStation = async function(req, res) {
  //  var station = await stationController.addStation(req.body.station_name);
  let station = await stationController.setIsPrivateOfStation(
    req.params.id,
    req.body.user_id,
    req.body.value,
  );
...
station.addSong = async function(req, res) {
  let station = await stationController.addSong(
    req.params.id,
    req.body.url,
    req.body.user_id,
  );
...
station.updateStartingTime = async function(req, res) {
  let station = await stationController.updateStartingTime(
    req.params.id,
...
station.setPlayedSongs = async function(req, res) {
  let station = await stationController.setPlayedSongs(
    req.params.id,
    req.body.songIds,
  );
  res.status(200).json(station);
...
station.upVote = async function(req, res) {
  let song = await stationController.upVote(
    req.params.id,
    req.body.song_id,
    req.body.user_id,
  );
...
station.downVote = async function(req, res) {
  let song = await stationController.downVote(
    req.params.id,
    req.body.song_id,
    req.body.user_id,
  );
...
station.getListStationUserAddedSong = async function(req, res) {
  let song = await stationController.getListStationUserAddedSong(
    req.body.user_id,
  );
  res.json(song);
};
